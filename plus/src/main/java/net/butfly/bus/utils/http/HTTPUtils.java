...
public class HTTPUtils extends UtilsBase {
	...
	public static CloseableHttpClient createFull(int connTimeout, int readTimeout, int maxConnections) {
		return createFull(connTimeout, readTimeout, maxConnections, -1, null);
	}
	...
	public static CloseableHttpClient createFull(int connTimeout, int readTimeout, int maxConnections, int maxPerRoute,
		...
		// initialization of outgoing HTTP
		// connections. Beside standard connection configuration parameters HTTP
		// connection factory can
		...
		// socket. The process of network socket initialization, its connection
		// to a remote address and
		...
		// by default or for a specific
		// host.
		connManager.setDefaultSocketConfig(socketConfig);
		// XXX: connManager.setSocketConfig(new HttpHost("somehost", 80),
		// socketConfig);
		...
		// either by default or for a
		// specific host.
		connManager.setDefaultConnectionConfig(connectionConfig);
		// XXX: connManager.setConnectionConfig(new HttpHost("somehost", 80),
		...
		RequestConfig defaultRequestConfig = RequestConfig.custom().setCookieSpec(CookieSpecs.BEST_MATCH)
				...
				.setConnectTimeout(connTimeout).setConnectionRequestTimeout(readTimeout).build();
